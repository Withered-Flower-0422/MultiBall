...
const addBallAvatarTexAssets = () => {
  const paths = builtinBalls.map(
    (ball) => `Textures/Balls/${ball}.tex`
  );
...
export const execute = () => {
  if (!addBallAvatarTexAssets()) {
    dialogWindowManager.openMessageDialog(
      ...
      'Some ball avatar texture files are missing. Please check "Assets/Textures/Balls/" folder.',
  ...
  if (!addMultiBallItemToScene()) {
    dialogWindowManager.openMessageDialog(
      ...
      '"MultiBall.item" file is missing. Please add it to "Assets/Items/MultiBall/" folder.',
      editor.language === "Chinese" ? "确认" : "Ok",
      () => {}
    );
    return;

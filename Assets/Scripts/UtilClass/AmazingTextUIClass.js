...
export default class AmazingTextUI {
  ...
  constructor(text, fontSize, offset, defaultShow, animeDuration, color, allowMouseRightToggle) {
    ...
    this.ui.onPointerClick = mouseButton => {
      if (mouseButton === 0 && !this.duringConfig && !this.duringAnimation) {
  ...
  show(text) {
    ...
    tweenManager.createFloatTween(0, text.length, "Linear", this.animeDuration, v => this.setText(text.slice(0, v)), () => {
      this.setText(text);
  ...
  hide() {
    ...
    tweenManager.createFloatTween(text.length, 0, "Linear", this.animeDuration, v => this.setText(text.slice(0, v)), () => {
      this.setText("");
  ...
  update(text) {
    if (this.allowMouseRightToggle && inputManager.mouse.checkButtonDown("Right")) this.toggle(text);
    if (this.ui.enabled && !this.duringAnimation && this.text !== text) this.setText(text);
  }
}
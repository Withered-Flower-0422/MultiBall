...
export default class Manager {
  ...
  sendEvent(name, data) {
    levelManager.sendCustomEvent(_defineProperty(_defineProperty({}, this.eventSymbol, true), name, data));
  }
  isSelfEvent(e) {
  ...
  update(e) {
    ...
    this.onEvents(e);
    if (e.OnStartLevel) Object.values(this.keys).forEach(key => key.updateConflictKey());
    if (e.OnPhysicsUpdate) Object.values(this.keys).forEach(key => key.update());

...
export default class CustomKey {
  ...
  constructor(keyName, key, disableConflictKey, fontSize, offset, defaultShow, animeDuration, color, allowMouseRightToggle) {
    ...
    this.keyName = typeof keyName === "string" ? {
      English: keyName,
      Spanish: keyName,
      日本語: keyName,
      简体中文: keyName,
  ...
  checkDown() {
    return this.ui.duringConfig ? false : this.isKeyboardKey() ? inputManager.keyboard.checkKeyDown(this.key) : inputManager.mouse.checkButtonDown(this.key);
  }
  checkHold() {
    return this.ui.duringConfig ? false : this.isKeyboardKey() ? inputManager.keyboard.checkKeyHold(this.key) : inputManager.mouse.checkButtonHold(this.key);
  }
  checkUp() {
    return this.ui.duringConfig ? false : this.isKeyboardKey() ? inputManager.keyboard.checkKeyUp(this.key) : inputManager.mouse.checkButtonUp(this.key);
  }
  changeKey(key) {
  ...
  updateConflictKey() {
    if (this.disableConflictKey) {
      for (const k of allPlayerKeys) {
        if (inputManager.getPlayerKey(k) === this.key) inputManager.disablePlayerKey(k);
      }
    }
  }
  update() {
    if (this.ui.duringConfig) {
      for (const key of [...allKeyboardKeys, "Left", "Right", "Middle"]) {
        if (checkKeyDown(key)) {
          this.changeKey(key);

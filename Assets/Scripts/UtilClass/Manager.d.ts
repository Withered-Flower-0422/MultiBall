...
export default abstract class Manager<Event extends {
    _brand: symbol;
} = {
    _brand: symbol;
}, TipKey extends string = never> {
    protected readonly eventSymbol: symbol;
    protected canceledEvents: Set<keyof Event & `OnPre${string}`>;
    protected sendEvent<T extends Exclude<keyof Event, "_brand">>(name: T, data: Event[T]): void;
    /**
     * Checks whether the given object is an event of this manager.
     * @param e - The object to check.
     * @returns Whether the object is an event of this manager.
     */
    isSelfEvent(e: any): e is Event;
    /**
     * Cancels the given event.
     * @param event - The event to cancel.
     */
    cancelEvent(event: keyof Event & `OnPre${string}`): void;

// @ts-nocheck
import { math, Float2, Float3, Quaternion } from "gameApi";export let

mathEx;(function (_mathEx) {
  ...
  const clamp = _mathEx.clamp = (value, min, max) =>
  ...
  const normalizeDeg = _mathEx.normalizeDeg = (degrees) =>
  (degrees % 360 + 360) % 360;
  ...
  const lengthFloat2 = _mathEx.lengthFloat2 = (value) =>
  ...
  const scaleFloat2 = _mathEx.scaleFloat2 = function (value) {
    let { x, y } = value;for (var _len = arguments.length, scalars = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {scalars[_key - 1] = arguments[_key];}
  ...
  const addFloat3 = _mathEx.addFloat3 = function () {
    let x = 0;
    let y = 0;
    let z = 0;for (var _len2 = arguments.length, values = new Array(_len2), _key2 = 0; _key2 < _len2; _key2++) {values[_key2] = arguments[_key2];}
  ...
  const subFloat3 = _mathEx.subFloat3 = function (a) {
    let { x, y, z } = a;for (var _len3 = arguments.length, b = new Array(_len3 > 1 ? _len3 - 1 : 0), _key3 = 1; _key3 < _len3; _key3++) {b[_key3 - 1] = arguments[_key3];}
    for (const v of b) {
      x -= v.x;
      y -= v.y;
      z -= v.z;
  ...
  const scaleFloat3 = _mathEx.scaleFloat3 = function (value) {
    let { x, y, z } = value;for (var _len4 = arguments.length, scalars = new Array(_len4 > 1 ? _len4 - 1 : 0), _key4 = 1; _key4 < _len4; _key4++) {scalars[_key4 - 1] = arguments[_key4];}
    for (const s of scalars) {
      x *= s;
      y *= s;
      z *= s;
  ...
  const invertQuaternion = _mathEx.invertQuaternion = (q) =>
  new Quaternion(-q.x, -q.y, -q.z, q.w);

  const _mulQuaternion = (a, b) =>
  new Quaternion(
  ...
  const dotQuaternion = _mathEx.dotQuaternion = (a, b) =>
  ...
  const negQuaternion = _mathEx.negQuaternion = (q) =>
  new Quaternion(-q.x, -q.y, -q.z, -q.w);
  ...
  const lengthQuaternion = _mathEx.lengthQuaternion = (q) =>
  ...
  const getAngularVelocityToUnit = _mathEx.getAngularVelocityToUnit = function (q) {let kp = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 8;
    let { x, y, z, w } = q;
  ...
  const getAngularVelocityToTarget = _mathEx.getAngularVelocityToTarget = function (
  a)
  ...
  const float3ToPureQuaternion = _mathEx.float3ToPureQuaternion = (v) =>
  new Quaternion(v.x, v.y, v.z, 0);
  ...
  const quaternionToAxisAngle = _mathEx.quaternionToAxisAngle = (q) => {
    ...
    if (s < 1e-9) return { axis: new Float3(0, 1, 0), angle: 0 };
    return {
  ...
  const rotPosAroundAxis = _mathEx.rotPosAroundAxis = (
  currentPos,
  pivot,
  axis,
  angle) =>
  ...
  const rotQuatAroundAxis = _mathEx.rotQuatAroundAxis = (
  currentQuat,
  axis,
  angle) =>
  ...
  const getAccelerationByForceAtPoint = _mathEx.getAccelerationByForceAtPoint = (
  itemPos,
  itemQuat,
  forcePos,
  forceVec,
  ...
  const getVelocityByForceAtPoint = _mathEx.getVelocityByForceAtPoint = (
  itemPos,
  itemQuat,
  forcePos,
  forceVec,
